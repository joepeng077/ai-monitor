...
> 状态：公开版 `1.1.2`。它可以显示 Mac 系统状态，并提供 Codex、Claude、WorkBuddy 三张默认**不读取账户登录态**的小组件。

## 1.1.2 更新

- 固化套餐显示的单一来源：公开版只展示安全 provider 或本地脱敏导入提供的 `accountPlan`。
- 当前 `accountPlan` 优先于旧版 `planLabel`；当前值无效时隐藏标签，不回退为旧付费套餐。
- 不保留本机套餐覆盖，账户自动读取继续保持关闭。
